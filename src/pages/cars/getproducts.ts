...
import { Car, MasterData } from '../../types/types'
...
export const createCarsList = (carsArr: Car[], carsCardContainer: HTMLElement): HTMLElement => {
  for (const carData of carsArr) {
    const carCardContainer = createHtmlElement({ tagName: 'div', cssClass: [CSS_CLASSES.carCard], })
    ...
    const oneCarData = carData.masterData.current.name["en-US"];
    oneCarData
...
const carCharacterBlock = (carData: Car, atributesContainer: HTMLElement) => {
  ...
  for (const attribute of arrayAtributs) {
    ...
    if (attributeName === 'number-of-seats' || attributeName === 'engine-power' || attributeName === 'max-speed') {
      ...
      const atributeName = createHtmlElement({
        tagName: 'span',
        cssClass: [CSS_CLASSES.carCharacterCont],
        elementText: attributeName.charAt(0).toUpperCase() + attributeName.slice(1).replace(/-/g, " "),
      })
